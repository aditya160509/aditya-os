...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      // No mail backend on a static site: hand the composed message to the
      // visitor's own mail client instead of posting it anywhere.
      const subject = encodeURIComponent(`Portfolio message from ${fullName}`);
      const body = encodeURIComponent(`${message}\n\n— ${fullName} (${email})`);
      window.location.href = `mailto:${config.email}?subject=${subject}&body=${body}`;
      toast({
        title: "Opening your mail app",
        description: "Your message is ready to send — hit send and it reaches me.",
        variant: "default",
